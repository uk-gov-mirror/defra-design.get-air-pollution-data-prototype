...
// Set dimensions and margins for the chart
  function drawChart() {
    ...
    d3.select("#no2-container-2024").selectAll("*").remove();
  
    // Set dimensions and margins for the chart
    const margin = { top: 20, right: 10, bottom: 40, left: 40 };
    
    // Use the full width of the container
    const width = document.getElementById("no2-container-2024").clientWidth - margin.left - margin.right; // Use container's width
...
const svg = d3.select("#no2-container-2024")
  .append("svg")
...
d3.csv("/public/javascripts/version_13/chart-data/2024/no2.csv").then(function (data) {
...
function handleFocus(event, d) {
  ...
  const container = document.getElementById("no2-container-2024");
...
function handleKeydown(event, d) {
  const circles = document.querySelectorAll("#no2-container-2024 .focus-points circle");
...
d3.select("#no2-container-2024").on("focus", function () {
  ...
  const allCircles = document.querySelectorAll("#no2-container-2024 .focus-points circle");
...
document.addEventListener("click", function (event) {
  ...
  if (window.innerWidth > 768) {
    ...
    d3.select("#no2-container-2024").select(".focus-points").remove();
...
// Create event listeners for the no2-container-2024 to show/hide elements
d3.select("#no2-container-2024")
...
window.AQGraphs.no2_2024 = drawChart; // name it whatever you like

// Initial chart rendering (only runs if the container exists)
if (document.getElementById("no2-container-2024")) {
  drawChart();
}
