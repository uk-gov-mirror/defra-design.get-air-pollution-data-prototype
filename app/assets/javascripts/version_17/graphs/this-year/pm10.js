...
// Set dimensions and margins for the chart
  function drawChart() {
    ...
    d3.select("#pm10-container-2026").selectAll("*").remove();
  
    // Set dimensions and margins for the chart
    const margin = { top: 20, right: 10, bottom: 40, left: 40 };
    
    // Use the full width of the container
    const width = document.getElementById("pm10-container-2026").clientWidth - margin.left - margin.right; // Use container's width
...
const svg = d3.select("#pm10-container-2026")
  .append("svg")
...
d3.csv("/public/javascripts/version_13/chart-data/2026/pm10.csv").then(function (data) {
...
function handleFocus(event, d) {
  ...
  const container = document.getElementById("pm10-container-2026");
...
function handleKeydown(event, d) {
  const circles = document.querySelectorAll("#pm10-container-2026 .focus-points circle");
...
d3.select("#pm10-container-2026").on("focus", function () {
  ...
  const allCircles = document.querySelectorAll("#pm10-container-2026 .focus-points circle");
...
document.addEventListener("click", function (event) {
  ...
  if (window.innerWidth > 768) {
    ...
    d3.select("#pm10-container-2026").select(".focus-points").remove();
...
// Create event listeners for the pm10-container-2026 to show/hide elements
d3.select("#pm10-container-2026")
...
window.AQGraphs.pm10_2026 = drawChart; // name it whatever you like

// Initial chart rendering (only runs if the container exists)
if (document.getElementById("pm10-container-2026")) {
  drawChart();
}
